...
程式碼功能
    ...
    2. 將補 0 後的數據子載波轉換成矩陣
    3. 將 8*8 的矩陣放入 10*10 的矩陣，使 8*8 矩陣的四周都有 0
        0000000000
        0  8*8   0
        :        :
        0000000000
    4. 將矩陣轉換成圖片並儲存，另外還可以等比例放大圖片
參考 :
...
if __name__ == "__main__":
    ...
    """
    FILE_NAME1 = "C:/PythonProject/plot_csi_figure/src/old/CSI_DATA/0608/walk.csv"                    # 原始 csv 檔案
    PATH = "C:/PythonProject/plot_csi_figure/src/old/dataset/0608/train/"

    f1 = open(FILE_NAME1)

    loop1 = 0
    loop_n = 4      # 設定總共要輸出多少筆資料
    ...
    for j1, l1 in enumerate(f1.readlines()):
        if j1 >= j0:        # 因為 j1 是從 0 開始計算，如果要讀取 20 行，需要將 j1 設為  19
            ...
            # 將原始的8x8矩陣放入新的10x10矩陣中
            matrix_10x10[1:9, 1:9] = matrix
            ...
            for i in range(size[0]):
                for j in range(size[1]):
                    pixels[i, j] = int(float(matrix_10x10[j][i]))
            ...
            # 儲存圖片
            address = PATH + "manned-{}.png".format(j1)   # 編輯圖片的名稱
